...
//		List<Goods> list2 = goodsService.selectAll();
//		for(Goods g : list2) {
//		saveBuyOption(itemcode, itemname, g.getRnum());
//		break;
//		}
		return "redirect:/";
	}
...
//		}
	...
	public String create(@ModelAttribute BuyOption buyOption,
			@RequestParam(value="item_index") int[] item_index,
							HttpServletRequest request) {
		System.out.println(item_index);
		for(int i=0; i<item_index.length; i++) {
			Map<String, Object> map = new HashMap();
			map.put("buyOptionCode", item_index[i]);
//			map.put("rnum", item_index[i]);
//			map.put("optionName", optionName);
//			map.put("optionPrice", optionPrice);
		//	buyOptionService.insert(map);	
		}
