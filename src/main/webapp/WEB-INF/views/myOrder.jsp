        	...
        	<div class="cf">        	
	        	...
	        	<form name="form1" method="get">
					...
					<c:when test="${empty payInfo}">
					    ...
					    <c:otherwise>
						<c:forEach var="list2" items="${orderInfo}" varStatus="status">
						
						<table class="type09" style="width:100%">
						<tr>	
							<td colspan="6">주문 날짜: <fmt:formatDate value="${list2.dealDate}" pattern="yyyy-MM-dd"/></td>
						</tr>
						
						<tr>
							<td><img src="${pageContext.request.contextPath}/${goods[status.index].image}" style="width:300px"/></td>
							<td colspan="2"><a href='detail?rnum=${list2.rnum}'>${goods[status.index].goodsName}</a></td>  						
						</tr>
						
						<c:forEach items="${dealCode}" var="dealCode">
						<tr>
							<td style="text-align:center">${dealCode.optionName} </td>	
							<td style="text-align:center">${dealCode.saleOptionCount} 개</td>
							<td style="text-align:center"><fmt:formatNumber value="${dealCode.optionPrice}" pattern="#,###"/> 원</td>							
						</tr>
						</c:forEach>
						<tr>
							<td></td>
							...
							<fmt:formatNumber value="${list2.totalPrice}" pattern="#,###"/> 원 
							(+ <fmt:formatNumber value="${list2.payDeliveryCharge}" pattern="#,###"/> 원 )			
							</td>				
						</tr>
						</table><p><p>
						</c:forEach>
